...
class Main: #CRIAÇÃO DA FUNÇÃO PRINCIPAL, QUE CONTERÁ AS DEMAIS FUNÇÕES
    ...
    def extracao_dados(self, filtro_chave_1, filtro_chave_2, dicio): # VARRE OS DICIONÁRIOS DA LISTA 'menu' E RETORNA 2 VALORES EM UM NOVO DICIONÁRIO
        ...
        for item in MENU: # SEPARA CADA UM DOS ITEMS DA LISTA 'menu' (CADA ITEM É 1 DICIONÁRIO)
    ...
    def opcao_criar_usuario(self, lista_usuario):
        cpf = print(input('Digite o seu CPF: '))
    ...
    def verific_conta(self):
        for item in lista_usuario:
            for chave03, valor03 in item.items():
                if chave03 == 
    ...
    def tela_inicial(self): # NOVA FUNÇÃO PARA EXIBIR A TELA INICIAL
        self.extracao_dados('Opção', 'Relação', self.menu01) # CRIA UMA NOVA VARIÁVEL PARA A FUNÇÃO 'extracao_dados'
        ...
        self.extracao_dados('Opção', 'Função', self.tela01)
